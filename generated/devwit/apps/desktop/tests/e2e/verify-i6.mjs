...
try {
  ...
  if (traceB !== undefined) {
    ...
    // AC35 起 done 前记 usage 事件（真实计量可审计）：同属附加观测事件，一并剔除
    const core = types.filter((type) => type !== "route" && type !== "usage");
    assert(
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "4. 中断续跑：interrupted 任务输入可用；user_message 事件使其复活为 running；轨迹文件追加不覆盖（seq 连续；AC31 起每次 run 首记 route 事件、AC35 起 done 前记 usage 事件，核心序列剔除后校验）。",
