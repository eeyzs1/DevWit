...
**最新版本 v0.7.30 · 免费软件 · MIT 协议**（全部构建产物见 [Releases](https://github.com/eeyzs1/DevWit/releases)）

### Windows（x64）

直接下载：[DevWit.Setup.0.7.30.exe](https://github.com/eeyzs1/DevWit/releases/download/v0.7.30/DevWit.Setup.0.7.30.exe)（NSIS 安装包，可选安装目录，支持 `/S` 静默安装）。
...
winget（microsoft/winget-pkgs#407506、#422497、#425984 已先后合并，最新 v0.7.0 已上线；v0.7.30 清单已备于 distribution/winget 待提交）：
...
或直接下载：[DevWit-0.7.30-arm64.dmg](https://github.com/eeyzs1/DevWit/releases/download/v0.7.30/DevWit-0.7.30-arm64.dmg)（Intel Mac 暂无构建）。

### Linux（x64）

- AppImage（支持应用内自动更新）：[DevWit-0.7.30.AppImage](https://github.com/eeyzs1/DevWit/releases/download/v0.7.30/DevWit-0.7.30.AppImage)，下载后 `chmod +x` 直接运行
- Debian/Ubuntu：[devwit_0.7.30_amd64.deb](https://github.com/eeyzs1/DevWit/releases/download/v0.7.30/devwit_0.7.30_amd64.deb)，`sudo dpkg -i` 安装
...
| 终端面板 | 侧栏内置真实 shell 会话（pty/pipe 双后端），ANSI 彩色输出流式渲染、方向键/快捷键完整传递、进程树击杀、会话重启一键完成 |
| 命令面板 | Ctrl+Shift+P 执行命令、Ctrl+P 打开文件（VS Code 惯例）；模糊过滤 + 键盘导航闭环 |
