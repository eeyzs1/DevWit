...
export function localizeError(raw: string, opts?: LocalizeErrorOptions): string {
  ...
  // v0.7.20（审查 R4）：凭证解密失败（换机/重装后密钥不可用）——本地化提示重录
  if (raw.includes("DW_CREDENTIAL_DECRYPT_FAILED")) return t("err.credentialDecryptFailed");
  ...
  if (raw.includes("DW_GIT_STASH_FAILED")) return t("err.gitStashFailed");
