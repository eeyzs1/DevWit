...
上下文不是越多越好。DevWit 内置任务路由器：简单请求（改错别字、小问题）走本地
Ollama 模型——免费、私有、可离线；复杂多文件任务走你配置的云端模型（Anthropic
或任意 OpenAI 兼容 API）。每次会话的用量账本记录 token 与估算成本，路由决策本身
也可审计。
...
- **TypeScript 语言服务器**：实时诊断、悬停、跳转定义
- **集成 Git**：文件树状态徽标、暂存/提交、内联 diff
...
v0.3.0 由 **614 个单元测试和 28 套端到端测试**验证（驱动真实打包产物，不是
...
等我开通账号）、winget 分发（PR 审核中），然后是更多语言的 LSP 覆盖。

如果你在有合规/审计需求的团队，或者只是想知道自己的 AI 工具在干什么——
真心欢迎反馈。

**链接**：[GitHub](https://github.com/eeyzs1/DevWit) ·
[下载 v0.3.0](https://github.com/eeyzs1/DevWit/releases/latest)
