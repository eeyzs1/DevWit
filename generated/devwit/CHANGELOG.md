...
### Added
- **命令面板**：Ctrl+Shift+P 命令 / Ctrl+P 文件（VS Code 惯例）——
  - 模糊过滤（`palette-filter.ts` 纯函数，9 项单测：子序列匹配 + 连续/
    词首加分 + 命中密度；空格宽松分隔；大小写不敏感）
  - 命令目录：打开文件夹/保存/设置/形态切换/搜索切换/五个面板跳转/
    外部编辑器（本地化命令名，语言热生效）
  - 键盘闭环：↑↓ 导航 + Enter 执行 + Escape 关闭（v0.7.29 a11y 口径）；
    鼠标悬停同步高亮
  - E2E verify-palette 4 断言（命令列表/过滤执行到终端/Esc 关闭/文件
    面板过滤打开）首跑全绿

## [0.7.29] — 2026-10-02

### Fixed
- **R8-10 a11y 收尾**（八轮审查最后一项）：
  - **Escape 关闭**：设置页 / 编辑器引导 / 两个导览（Escape = 确认关闭，
    stopPropagation 不穿透）与向导（Escape = 跳过语义，幂等闩锁保护）全部
    接入——旧实现无任何 Esc 路径，键盘用户必须精确点击关闭按钮
  - **打开即聚焦**：向导聚焦首个交互控件、设置页聚焦关闭按钮、导览聚焦
    Got it（Enter/Escape 键盘闭环）、编辑器引导保持输入框聚焦
